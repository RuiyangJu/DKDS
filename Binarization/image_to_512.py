...
def main(argv=None):
    ...
    image_dir = './Trainset/image/'
    mask_dir = './Trainset/mask/'
    ...
    image_save_dir = './Resize_512/Trainset/image/'
    mask_save_dir = './Resize_512/Trainset/mask/'
    ...
    for image_path in tqdm(image_pathes, desc="Processing images"):
        ...
        scale_cnt = 0
        for scale in tqdm(scale_list, desc=f"Scaling {image_name}", leave=False):
            crpW = int(scale * imgw)
            crpH = int(scale * imgh)
            ...
            for idx in tqdm(range(len(image_patches)), desc=f"Saving patches {image_name} s{scale_cnt}", leave=False):

...
def get_drd(im, im_gt):
    ...
    u_Resized[ic : xm+ic, jc : ym+jc] = im[:, :, 0]
...
def get_metric(pred_img, gt_mask):
    ...
    tp[(pred_img[:,:,0]==0) & (gt_mask==0)] = 1
    numtp = tp.sum()

    fp = np.zeros(gt_mask.shape, np.uint8)
    fp[(pred_img[:,:,0]==0) & (gt_mask==1)] = 1
    numfp = fp.sum()

    fn = np.zeros(gt_mask.shape, np.uint8)
    fn[(pred_img[:,:,0]==1) & (gt_mask==0)] = 1
    numfn = fn.sum()
    ...
    ptp[(pred_img[:, :, 0] == 0) & (im_sk == 0)] = 1
    numptp = ptp.sum()

    pfn = np.zeros(gt_mask.shape, np.uint8)
    pfn[(pred_img[:, :, 0] == 1) & (im_sk == 0)] = 1
    numpfn = pfn.sum()

    precall = numptp / float(numptp + numpfn)
    pfmeasure = 100 * (2 * precall * precision) / (precall + precision) 

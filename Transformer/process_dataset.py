...
def prepare_dibco_experiment(input_root, output_root, train_folder, val_folder, test_folder, patch_size_train, patch_size_valid, overlap_size):
    """
    Process DIBCO-like dataset and create patches.

    Args:
        input_root (str): input root folder containing train/valid/test folders
        output_root (str): output root folder
        train_folder (str): folder name for training data
        val_folder (str): folder name for validation data
        test_folder (str): folder name for testing data
        patch_size_train (int): patch size for training (augmented)
        patch_size_valid (int): patch size for validation/testing
        overlap_size (int): overlap for training patches
    """
...
if __name__ == "__main__":
    # 設定參數
    input_root = './dataset'  # 原始資料集所在資料夾
    output_root = './processed'  # 輸出資料夾
    train_folder = 'train'
    val_folder = 'valid'
    test_folder = 'test'
